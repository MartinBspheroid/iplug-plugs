...
def  main():
  ...
  installer = "\installer\SRC Installer.exe"
   
  if demo:
    installer = "\installer\SRC Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRC manual.pdf" 
  ]
  ...
  ZIPNAME = "SRC-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRC-v" + FULLVERSIONSTR + "-win-demo.zip"
