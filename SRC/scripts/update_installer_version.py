...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=SRC Demo Installer\n"
      else:
        line="OutputBaseFilename=SRC Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the SRC Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the SRC installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=SRC Demo installer\n"
     else:
       line="SetupWindowTitle=SRC installer\n"
