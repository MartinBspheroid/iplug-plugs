...
__m128 VecDiode::process(__m128 x) {
    __out_ = _mm_and_ps(vecCalcNLP(x, _vB, _vLScaled),_mm_cmpgt_ps(x, _mm_set1_ps(_vB)));
    __out_ = _mm_switch_ps(__out_, vecCalcLin(x, _vB, _vLScaled), _mm_cmpgt_ps(x, _mm_set1_ps(_vLScaled)));
    return _mm_mul_ps(__out_, _mm_set1_ps(_hB));
}
