...
Amalgam::Amalgam() {
    ...
    xGain = 1.f;
    yGain = 1.f;
    vecAmalgam.setMode(2);
    /// IMPORTANT!!!!!!!
   
}

void Amalgam::process(double leftX, double rightX, double leftY, double rightY) {
  ...
  /*  paramB += inputs[PARAM_B_CV1_INPUT].getVoltage() * 0.1f * params[PARAM_B_CV1_PARAM].getValue();
   ...
   /* if(inputs[X_LEFT_INPUT].isConnected() && !inputs[X_RIGHT_INPUT].isConnected()) {
    ...
    //xGain =  params[X_GAIN_CV_PARAM].getValue() * 0.4f;
    ...
    //yGain =  params[Y_GAIN_CV_PARAM].getValue() * 0.4;
    ...
    outputL = (tanhDriveSignal(zOut[0], 0.66f) * 5.5f);
    outputR = (tanhDriveSignal(zOut[2], 0.66f) * 5.5f);
}
