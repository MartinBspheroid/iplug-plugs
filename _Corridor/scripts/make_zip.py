...
def  main():
  ...
  installer = "\installer\Corridor Installer.exe"
   
  if demo:
    installer = "\installer\Corridor Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Corridor manual.pdf" 
  ]
  ...
  ZIPNAME = "Corridor-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Corridor-v" + FULLVERSIONSTR + "-win-demo.zip"
