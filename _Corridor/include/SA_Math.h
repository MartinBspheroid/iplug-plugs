...
*/
inline int clampSafe(const int &x, const int &a, const int &b) {
...
*/
inline int eucMod(const int &a, const int &b) {
	int mod = a % b;
	if (mod < 0) {
		mod += b;
	}
...
`b` must be positive.
*/
inline int eucDiv(const int &a, const int &b) {
	int div = a / b;
	int mod = a % b;
...
inline void eucDivMod(const int &a, const int &b, int* div, int* mod) {
	*div = a / b;
	*mod = a % b;
	if (*mod < 0) {
		*div -= 1;
...
If `b < a`, switches the two values.
*/
inline float clampSafe(const float &x, const float &a, const float &b) {
	return (a <= b) ? clamp(x, a, b) : clamp(x, b, a);
}

/** Converts -0.f to 0.f. Leaves all other values unchanged. */
inline float normalizeZero(const float &x) {
	return x + 0.f;
}
...
See https://en.wikipedia.org/wiki/Euclidean_division.
*/
inline float eucMod(const float a, const float b) {
...
inline bool isNear(const float &a, const float &b, const float &epsilon = 1e-6f) {
	return std::fabs(a - b) <= epsilon;
}

/** If the magnitude of `x` if less than epsilon, return 0. */
inline float chop(const float x, const float epsilon = 1e-6f) {
	return std::fabs(x) <= epsilon ? 0.f : x;
}

inline float rescale(const float &x, const float &xMin, const float &xMax, const float &yMin, const float &yMax) {
	return yMin + (x - xMin) / (xMax - xMin) * (yMax - yMin);
}

inline float crossfade(const float &a, const float &b, const float &p) {
	return a + (b - a) * p;
}
...
*/
inline float interpolateLinear(const float* p, const float &x) {
	int xi = x;
	float xf = x - xi;
...
*/
inline void complexMult(const float &ar, const float &ai, const float &br, const float &bi, float* cr, float* ci) {
	*cr = ar * br - ai * bi;
	*ci = ar * bi + ai * br;
}
