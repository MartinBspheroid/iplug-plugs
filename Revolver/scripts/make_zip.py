...
def  main():
  ...
  installer = "\installer\Revolver Installer.exe"
   
  if demo:
    installer = "\installer\Revolver Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Revolver manual.pdf" 
  ]
  ...
  ZIPNAME = "Revolver-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Revolver-v" + FULLVERSIONSTR + "-win-demo.zip"
