...
public:
  ...
  std::vector<std::string> names = { "Input Trim", "Tape High", "Tape Low", "Flutter", "Noise", "Output Trim", "Inv/Dry/Wet" };
  // TODO: Implement this????
  ...
  void processReplacing(float** inputs, float** outputs, const int& nFrames);
  void processDoubleReplacing(double** inputs, double** outputs, const int& nFrames);
  float sampleRate = 0;
  float A;
  float B;
  float C;
...
void SA_AW_IronOxide::processReplacing(float** inputs, float** outputs,  const int&  nFrames)
{
...
void SA_AW_IronOxide::processDoubleReplacing(double** inputs, double** outputs, const int&  nFrames)
{
  int sampleFrames = nFrames;
  double* in1 = inputs[0];
  double* in2 = inputs[1];
