...
def  main():
  ...
  installer = "\installer\Smoke Installer.exe"
   
  if demo:
    installer = "\installer\Smoke Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Smoke manual.pdf" 
  ]
  ...
  ZIPNAME = "Smoke-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Smoke-v" + FULLVERSIONSTR + "-win-demo.zip"
