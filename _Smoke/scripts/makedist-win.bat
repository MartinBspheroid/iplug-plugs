...
if %1 == 1 (echo Making Smoke Windows DEMO VERSION distribution ...) else (echo Making Smoke Windows FULL VERSION distribution ...)
...
REM - msbuild Smoke-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Smoke.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Smoke.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Smoke.aaxplugin\Contents\Win32\Smoke.aaxplugin --out .\build-win\aax\bin\Smoke.aaxplugin\Contents\Win32\Smoke.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Smoke.aaxplugin\Contents\x64\Smoke.aaxplugin --out .\build-win\aax\bin\Smoke.aaxplugin\Contents\x64\Smoke.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Smoke.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Smoke.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Smoke Installer" ".\installer\Smoke Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Smoke Installer.exe" ".\installer\Smoke Demo Installer.exe"
REM -del ".\installer\Smoke Installer.exe"
REM -)
