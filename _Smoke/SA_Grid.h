...
// this with result in even distribution in horiyontal axis, but
  ...
  IRECT getArea(const int left, const int top, const int right, const int bottom) {
    try
    {
      ...
      return base.GetOffset(_left, _top, _right, _bottom);
    }
...
// columnEnd & rowEnd (x,y)
  ...
  // @param scale -> get all IRECTs in collection scaled from center by fraction. 
  // Default is 1.0 for non-scaled 
  std::vector<IRECT> getAllCells(float scale = 1.0) {
    std::vector<IRECT> cells;
    for (size_t y = 0; y < rows.size(); y++)
    {
      for (size_t x = 0; x < columns.size(); x++)
      {
        cells.push_back(getArea(x, y, x + 1, y + 1).GetScaledAboutCentre(scale));
      }
    }
    return cells;
  }
