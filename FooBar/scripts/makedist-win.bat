...
if %1 == 1 (echo Making FooBar Windows DEMO VERSION distribution ...) else (echo Making FooBar Windows FULL VERSION distribution ...)
...
REM - msbuild FooBar-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild FooBar.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild FooBar.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\FooBar.aaxplugin\Contents\Win32\FooBar.aaxplugin --out .\build-win\aax\bin\FooBar.aaxplugin\Contents\Win32\FooBar.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\FooBar.aaxplugin\Contents\x64\FooBar.aaxplugin --out .\build-win\aax\bin\FooBar.aaxplugin\Contents\x64\FooBar.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\FooBar.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\FooBar.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "FooBar Installer" ".\installer\FooBar Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\FooBar Installer.exe" ".\installer\FooBar Demo Installer.exe"
REM -del ".\installer\FooBar Installer.exe"
REM -)
