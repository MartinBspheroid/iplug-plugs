...
if %1 == 1 (echo Making Escher Windows DEMO VERSION distribution ...) else (echo Making Escher Windows FULL VERSION distribution ...)
...
REM - msbuild Escher-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Escher.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Escher.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Escher.aaxplugin\Contents\Win32\Escher.aaxplugin --out .\build-win\aax\bin\Escher.aaxplugin\Contents\Win32\Escher.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Escher.aaxplugin\Contents\x64\Escher.aaxplugin --out .\build-win\aax\bin\Escher.aaxplugin\Contents\x64\Escher.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Escher.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Escher.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Escher Installer" ".\installer\Escher Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Escher Installer.exe" ".\installer\Escher Demo Installer.exe"
REM -del ".\installer\Escher Installer.exe"
REM -)
