...
def  main():
  ...
  installer = "\installer\Escher Installer.exe"
   
  if demo:
    installer = "\installer\Escher Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Escher manual.pdf" 
  ]
  ...
  ZIPNAME = "Escher-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Escher-v" + FULLVERSIONSTR + "-win-demo.zip"
