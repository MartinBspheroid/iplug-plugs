...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=Collider Demo Installer\n"
      else:
        line="OutputBaseFilename=Collider Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the Collider Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the Collider installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=Collider Demo installer\n"
     else:
       line="SetupWindowTitle=Collider installer\n"
