...
def  main():
  ...
  installer = "\installer\Collider Installer.exe"
   
  if demo:
    installer = "\installer\Collider Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Collider manual.pdf" 
  ]
  ...
  ZIPNAME = "Collider-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Collider-v" + FULLVERSIONSTR + "-win-demo.zip"
