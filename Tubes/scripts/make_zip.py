...
def  main():
  ...
  installer = "\installer\Tubes Installer.exe"
   
  if demo:
    installer = "\installer\Tubes Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Tubes manual.pdf" 
  ]
  ...
  ZIPNAME = "Tubes-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Tubes-v" + FULLVERSIONSTR + "-win-demo.zip"
