...
if %1 == 1 (echo Making Tubes Windows DEMO VERSION distribution ...) else (echo Making Tubes Windows FULL VERSION distribution ...)
...
REM - msbuild Tubes-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Tubes.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Tubes.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Tubes.aaxplugin\Contents\Win32\Tubes.aaxplugin --out .\build-win\aax\bin\Tubes.aaxplugin\Contents\Win32\Tubes.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Tubes.aaxplugin\Contents\x64\Tubes.aaxplugin --out .\build-win\aax\bin\Tubes.aaxplugin\Contents\x64\Tubes.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Tubes.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Tubes.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Tubes Installer" ".\installer\Tubes Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Tubes Installer.exe" ".\installer\Tubes Demo Installer.exe"
REM -del ".\installer\Tubes Installer.exe"
REM -)
